    ...
    cb(7) //建立棋盘
{
    ...
    connect(&tt, &TimeThread::addTime, this, &gameS7::updateTime);
...
void gameS7::setAccount(int i) //初始化界面
{
    account = i;
    reset();
    update();
...
void gameS7::continueGame(int i, Game g) //继续之前的游戏
{
    account = i;
    reset();
    lstscore = g.getScore();
...
void gameS7::keyPressEvent(QKeyEvent* e)
{
    if (dead) return;
...
void gameS7::closeEvent(QCloseEvent* event)
{
    if (!attempts || dead) {
        event->accept();
    }
...
void gameS7::on_pBtnRetry_clicked() //重新开始
{
    if (!attempts || dead) {
        reset();
        update();
...
void gameS7::on_pBtnRetry2_clicked() //重新开始（游戏失败后）
{
    reset();
    update();
}

void gameS7::on_pBtnPause_clicked() //暂停
{
    if (gaming) { //正在游戏中，暂停
        gaming = false;
        tt.terminate();
...
void gameS7::on_pBtnEnd_clicked() //结束游戏
{
    tt.terminate();
...
void gameS7::on_pBtnLast_clicked() //退回上一步
{
    tt.terminate();
...
void gameS7::on_pBtnHammer_clicked() //敲除一个方块
{
...
void gameS7::on_pBtnHome_clicked() //从游戏界面返回用户主界面
{
    systemclose = false;
    this->close();
}

void gameS7::on_pBtnHome2_clicked() //从游戏界面返回用户主界面（游戏失败后）
{
    systemclose = false;
    this->close();
}
